...
def usr(flyer):
    ...
    while True:
        ...
        #     except:
        #         print('msg has no length')
        # print(msgs)
        if msgs:
            for msg in msgs:
                print('attempting to unpack NEW...')
                unpacked_msg = struct.unpack('II',msg[0:8])
                print('unpacked message: ' + str(unpacked_msg))
                if unpacked_msg[1] == 42:
                    if not landing:
        ...
        #     if landing:
        #         msg = struct.pack('II', flyer.id, 42)
        #         flyer.send_msg(msg)
        #     else:
        #         msg = struct.pack('II', flyer.id, 0)
        #         flyer.send_msg(msg)
