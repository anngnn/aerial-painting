...
class FlightControllerInterface():
    ...
    def __init__(self, id, conn) -> None:
        ...
        #CONTROL THE FREQUENCY OF THE WHILE LOOP
        #Realistically, the controller is only updating at 50Hz anyway, so it doesn't make sense to update it faster than that
        self.ideal_frequency = 50 #hz
        self.ideal_loop_time = 1/self.ideal_frequency #seconds
        ...
        try:
            with MSPy(device=self.SERIAL_PORT, loglevel='WARNING', baudrate=115200) as board:
                ...
                while True:
                    ...
                    #if we were extra fast, we want to sleep (release CPU)
                    #on the Pis, sleeping 0.0009 was apprpoximately equal to sleeping 0.001 because of OS scheduling (when it decides to turn back on the process)
                    #Therefore, we will sleep for 80% of the calculated sleep time.
                    #Unlike on Windows, linux seems to be pretty accurate with sub second sleeps.
                    if loop_duration < self.ideal_loop_time:
                        rest_time = (self.ideal_loop_time - loop_duration) * 0.8
                        # print(loop_duration, rest_time, self.ideal_loop_time)
