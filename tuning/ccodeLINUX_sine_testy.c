...
int controlLoop(uint8_t *p_id, char *plocalizer_ip, uint16_t *plocalizer_port, uint16_t *plocalizer_timeout_sec, uint32_t *plocalizer_timeout_nsec,\
  ...
  *safety_code = 5; //DEVELOPERS NOTE: bevause safety_code is an array of length 1, I can use *safety_code OR safety_code[0]. They do the same thing.
  ...
  static float last_error_vel_y = 0.0;
  ...
  *shared_flight_mode = 1000;
  ...
  while (1) {
    ...
    if (file_initialized == FALSE && *user_code_running == TRUE){
      ...
      fprintf(fptr, "P_term_pos_y,I_term_pos_y,D_term_pos_y,desired_velocity_y,"); 
      ...
      fprintf(fptr, "P_term_vel_y,I_term_vel_y,D_term_vel_y,error_vel_y,d_error_vel_y,desired_roll_angle,");
      fprintf(fptr, "actual_vel_x,actual_vel_y,actual_pitch,actual_roll\n"); 
      ...
      if (*shared_controller_status == 1)
      {
        ...
        else if (pitch > 1.2 || pitch < -1.2)
        {
          ...
          *safety_code = 1;  //initiate a safety code 1 for estop.
          ...
          // **NEW: Override desired_velocity_y with sinusoidal test input**
          if (enable_velocity_test && (received_time - *user_code_start_time) > test_start_time) {
              ...
              desired_velocity_y = sine_amplitude * sin(2.0 * M_PI * sine_frequency * test_time);
        ...
        if (write_to_file){
          ...
          fprintf(fptr, "%0.3f,%0.3f,%0.3f,%0.3f\n", dx, dy, pitch, roll); 
