...
def usr(flyer):
    ...
    while True:
        ...
        #if this is the wand robot        
        if flyer.id == WAND_ID:
            flyer.disarm() #keep disarmed

            #get a position
            state = flyer.state()

            #If wwe actually get a state
            if len(state) > 1:
                try:
                    print(state)
                    x,y,z = state[0], state[1], state[2]

                    msg = struct.pack('Ifff', flyer.id, x, y, z)
                    flyer.send_msg(msg) #send the position to the other robot.
                    print('send: ', x, y, z)
                except:
                    continue

        #if this is the follower robot
        else:
            ...
            if msgs:
                for msg in msgs:
                    print('attempting to unpack NEW...')
                    unpacked_msg = struct.unpack('Ifff',msg[0:16])
                    print('unpacked message: ' + str(unpacked_msg))
                    if unpacked_msg[0] == WAND_ID: #if its from the wand robot
                        x, y, z = unpacked_msg[1], unpacked_msg[2], unpacked_msg[3]
                        setpoint[0] = x
                        setpoint[1] = y - 0.3 #always 30cm away from the other robot
                        setpoint[2] = z
                        flyer.waypoint(setpoint)
                        flyer.arm()
