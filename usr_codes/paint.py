...
def usr(flyer):
    ...
    for step in mission:
        ...
        if len(step) == 4:  # Gradual movement
            ...
            while remaining_dist > 0:
                current_step = min(step_size, remaining_dist)
                current_base = make_wp(current_base, dir_vec, current_step)
